...
import { usesNodeHooks, mergeNodeHooks, legacyShCommand, type HookTable } from '../utils/hook-command';
import type {
  CLIProvider,
...
export class GeminiProvider implements CLIProvider {
  ...
  async configureHooks(hooksDir: string, platform: NodeJS.Platform = process.platform): Promise<void> {
    ...
    if (usesNodeHooks(platform)) {
      this.configureNodeHooks(hooksDir);
      return;
    }
  ...
  /**
   * configureHooks on win32: the Node runner, on Gemini's own event names, and
   * the agent token let through Gemini's environment redaction.
   *
   ...
   */
  private configureNodeHooks(hooksDir: string): void {
    const settingsPath = path.join(this.configDir, 'settings.json');
    ...
    const specs = [
      ...
      ...rest, event: `gemini/${file.replace(/\.sh$/, '')}`, isLegacy: legacyShCommand(`gemini/${file}`),
    }));
